...
TEST_GROUP_RUNNER(vect_complex_mul) {
  ...
  RUN_TEST_CASE(vect_complex_mul, vect_complex_s16_mul_basic_quake);
  RUN_TEST_CASE(vect_complex_mul, vect_complex_s16_mul_random_quake);
...
static complex_s16_t mul_complex_s16_quake(complex_s16_t b, complex_s16_t c, int b_shr, int c_shr)
{
    int32_t bp_re = b.re;
    int32_t bp_im = b.im;
    int32_t cp_re = c.re;
    ...
    bp_re = ASHR(16)(bp_re, b_shr);
    bp_im = ASHR(16)(bp_im, b_shr);

    cp_re = ASHR(16)(cp_re, c_shr);
    cp_im = ASHR(16)(cp_im, c_shr);
    ...
    int64_t q1 = ((bp_re * cp_re) + (1<<13)) >> 14;
    int64_t q2 = ((bp_im * cp_im) + (1<<13)) >> 14;
    int64_t q3 = ((bp_re * cp_im) + (1<<13)) >> 14;
    int64_t q4 = ((bp_im * cp_re) + (1<<13)) >> 14;
...
TEST(vect_complex_mul, vect_complex_s16_mul_basic_quake)
{
    ...
    test_case_t casses[] = {
        ...
        { {{ 0x0000, 0x0000}, { 0x0000, 0x0000} },    {   0,   0}, { 0x0000, 0x0000}, __LINE__},
        { {{ 0x0001, 0x0001}, { 0x0000, 0x0000} },    {   0,   0}, { 0x0000, 0x0000}, __LINE__},
        { {{ 0x0000, 0x0000}, { 0x0001, 0x0001} },    { -20, -20}, { 0x0000, 0x0000}, __LINE__},
        { {{ 0x0001, 0x0000}, { 0x0001, 0x0000} },    { -7, -7}, { 0x0001, 0x0000}, __LINE__},
        { {{ 0x0001, 0x0000}, { 0x0001, 0x0000} },    { -8, -7}, { 0x0002, 0x0000}, __LINE__},
        { {{ 0x0001, 0x0000}, { 0x0001, 0x0000} },    { -7, -8}, { 0x0002, 0x0000}, __LINE__},
        { {{ 0x0001, 0x0000}, { 0x0000, 0x0001} },    { -7, -7}, { 0x0000, 0x0001}, __LINE__},
        { {{ 0x0001, 0x0001}, { 0x0001, 0x0000} },    { -7, -7}, { 0x0001, 0x0001}, __LINE__},
        // { {{ 0x0010, 0x0020}, { 0x0030, 0x0040} },    { -9, -9}, {-0x5000, 0xa000}, __LINE__},
        { {{ 0x4000, 0x0000}, { 0x4000, 0x0000} },    {   0,   0}, { 0x4000, 0x0000}, __LINE__},
        { {{ 0x0000, 0x4000}, { 0x0000, 0x4000} },    {   0,   0}, {-0x4000, 0x0000}, __LINE__},
    };
    ...
    for(int v = start_case; v < N_cases; v++){
        ...
        //Verify mul_complex_s16() is correct. It's used in other test cases.
        complex_s16_t tmp = mul_complex_s16_quake(casse->value.b, casse->value.c, casse->shr.b, casse->shr.c);
        ...
        for( int l = 0; l < sizeof(lengths)/sizeof(lengths[0]); l++){
            ...
            hr = vect_complex_s16_mul_quake(A, B, C, len, casse->shr.b, casse->shr.c);

            for(int i = 0; i < len; i++){
                TEST_ASSERT_EQUAL_MSG(casse->expected.re, A[0].re, casse->line);
                TEST_ASSERT_EQUAL_MSG(casse->expected.im, A[0].im, casse->line);
            }
            TEST_ASSERT_EQUAL_MSG(vect_s16_headroom((int16_t*) A, 2*len), hr, casse->line);

            memcpy(A, B, sizeof(A));
            hr = vect_complex_s16_mul_quake(A, A, C, len, casse->shr.b, casse->shr.c);
            ...
            memcpy(A, C, sizeof(A));
            hr = vect_complex_s16_mul_quake(A, B, A, len, casse->shr.b, casse->shr.c);
...
TEST(vect_complex_mul, vect_complex_s16_mul_random_quake)
{
    ...
    complex_s16_t B[MAX_LEN];
    complex_s16_t C[MAX_LEN];
    ...
    for(int v = 0; v < REPS; v++){
        ...
        for(int i = 0; i < len; i++){
            unsigned shr = pseudo_rand_uint32(&seed) % 8;
            B[i].re = pseudo_rand_int16(&seed) >> shr;
            C[i].re = pseudo_rand_int16(&seed) >> shr;
            B[i].im = pseudo_rand_int16(&seed) >> shr;
            C[i].im = pseudo_rand_int16(&seed) >> shr;
        }

        int b_shr = (pseudo_rand_uint32(&seed) % 10);
        int c_shr = (pseudo_rand_uint32(&seed) % 10);
        
        hr = vect_complex_s16_mul_quake(A, B, C, len, b_shr, c_shr);
        ...
        memcpy(&A, &B, sizeof(A));
        hr = vect_complex_s16_mul_quake(A, A, C, len, b_shr, c_shr);
        ...
        memcpy(&A, &C, sizeof(A));
        hr = vect_complex_s16_mul_quake(A, B, A, len, b_shr, c_shr);
