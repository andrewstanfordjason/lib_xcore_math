...
void ref_vcmr_s16(complex_s16_t * accu, complex_s16_t * vC, complex_s16_t * mem){
    for(int i=0;i<8;i++){

    }
}

void ref_vcmr_s32(complex_s32_t * accu, complex_s32_t * vC, complex_s32_t * mem){
    for(int i=0;i<4;i++){
        accu[i].re = vC[i].re*mem[i].re - vC[i].im*mem[i].im; 
    }
}
